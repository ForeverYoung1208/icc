class PartnersController < ApplicationController
  ...
  def set_selected_page
      @selected_page = 'partners'
  end

  def is_redactor
    partner = Partner.find(params[:id])
    if (current_user.nil? or not(current_user.redactor?( partner.section )) )
      redirect_to request.referer, :notice =>( t('partners.not_allowed')+' '+t(partner.section.name) )
    end
  end
